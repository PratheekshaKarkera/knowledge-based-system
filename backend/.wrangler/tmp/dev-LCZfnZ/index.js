...
// .wrangler/tmp/bundle-F0Tw9J/checked-fetch.js
...
// .wrangler/tmp/bundle-F0Tw9J/strip-cf-connecting-ip-header.js
...
var app = new Hono2();
app.use("/*", cors());
async function getAllArticles(kv) {
  const list = await kv.list({ prefix: "article:" });
  const articles = [];
  await Promise.all(
    list.keys.map(async (key) => {
      const article = await kv.get(key.name, "json");
      if (article)
...
__name(getAllArticles, "getAllArticles");
app.get("/", (c) => {
...
app.get("/api/articles/:id", async (c) => {
  ...
  let article = await c.env.KV.get(`article:${idOrSlug}`, "json");
  if (!article) {
    const allArticles = await getAllArticles(c.env.KV);
...
app.post("/api/articles", async (c) => {
  try {
    ...
    if (!title)
      return c.json({ error: "invalid request: title is required" }, 400);
    if (!category)
      return c.json({ error: "invalid request: category is required" }, 400);
    if (!content || content.length < 50)
      return c.json({ error: "invalid request: content minimum 50 characters" }, 400);
...
app.put("/api/articles/:id", async (c) => {
  try {
    ...
    const body = await c.req.json();
    const existing = await c.env.KV.get(`article:${id}`, "json");
    if (!existing)
      return c.json({ error: "article not found" }, 404);
    if (body.title === "")
      return c.json({ error: "invalid request: title cannot be empty" }, 400);
    if (body.category === "")
      return c.json({ error: "invalid request: category cannot be empty" }, 400);
    if (body.content && body.content.length < 50)
      return c.json({ error: "invalid request: content minimum 50 characters" }, 400);
    const updatedArticle = {
      ...existing,
      ...body,
      // override existing fields with new body fields
      id: existing.id,
      // don't override the primary ID
      updatedAt: Date.now()
    };
...
// .wrangler/tmp/bundle-F0Tw9J/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-F0Tw9J/middleware-loader.entry.ts
